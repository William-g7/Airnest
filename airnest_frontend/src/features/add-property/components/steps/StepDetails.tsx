...
import type { PropertyDetailsType } from '@addProperty/types';
...
export default function StepDetails({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        <TagSelector
          value={details.property_tags ?? []}
          onChange={(next) => setDetails({ ...details, property_tags: next })}
          min={1}
          max={5}
        />
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-2">{t('allPricesInUSD')}</p>
      </div>
    </div>
  );
}
