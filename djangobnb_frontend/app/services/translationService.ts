export const translateText = async (text: string, targetLanguage: string): Promise<string> => {
    ...
    try {
        console.log(`Translate text: "${text.substring(0, 30)}..."，target language: ${targetLanguage}`);
...
export const translateBatch = async (texts: string[], targetLanguage: string): Promise<Record<string, string>> => {
    ...
    try {
        ...
        if (data && data.translations) {
            console.log(`Batch translate success, received ${Object.keys(data.translations).length} results`);
