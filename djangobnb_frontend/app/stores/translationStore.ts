...
interface TranslationState {
    ...
    translateGrouped: (data: Record<string, string[]>, targetLocale?: string) => Promise<GroupedTranslationResponse>;
...
export const useTranslationStore = create<TranslationState>()((set, get) => ({
    ...
    translateMultiple: async (texts: string[], targetLocale?: string) => {
        ...
        try {
            ...
            set(state => ({
                pendingTranslations: state.pendingTranslations.filter(
                    t => !uncachedTexts.includes(t)
                )
            }));
        ...
        if (currentLocale === 'en') {
            ...
            Object.entries(data).forEach(([category, texts]) => {
                if (texts && texts.length > 0) {
                    result[category] = texts.reduce((obj, text) => {
                        obj[text] = text;
                        return obj;
                    }, {} as Record<string, string>);
                }
